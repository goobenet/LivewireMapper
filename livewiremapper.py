...
class LivewireGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Livewire Mapper (v5.3.0)
        ...
        if VLC_AVAILABLE:
            try:
                self.instance = vlc.Instance("--no-video --quiet --network-caching=300")
    ...
    def setup_ui(self):
        ...
        self.monitor_lbl = tk.Label(ctrl_frame, text="READY", bg="#1e1e1e", fg="#888888", font=("Consolas", 11)); self.monitor_lbl.pack(pady=2)
    ...
    def play_audio(self):
        ...
        self.player.set_media(self.instance.media_new(f"rtp://@{self.get_multicast_ip(ch)}:5004"))
        self.player.play(); self.meter_running = True; self.draw_meters()
    ...
    def toggle_record(self):
        if self.is_recording: self.stop_recording(); return
        sel = self.tree.selection()
        if not sel: return
        ...
        if path: self.start_recording(ch, path)
    ...
    def start_recording(self, ch, path):
        ...
        self.rec_player.set_media(self.rec_instance.media_new(f"rtp://@{self.get_multicast_ip(ch)}:5004", f":sout={sout}"))
        self.rec_player.play(); self.is_recording = True; self.recording_start_time = datetime.now()
        self.btn_record.config(bg="red", text="■ STOP REC"); self.update_timer()

    def stop_recording(self):
        if hasattr(self, 'rec_player'): self.rec_player.stop()
        self.is_recording = False; self.btn_record.config(bg="#444444", text="● RECORD"); self.timer_lbl.config(text="00:00")

    def update_timer(self):
        if not self.is_recording: return
        elapsed = (datetime.now() - self.recording_start_time).seconds
        if elapsed >= int(self.limit_var.get()) * 60: self.stop_recording(); return
    ...
    def do_node_trace(self):
        ...
        if s: self.trace_node_ip = self.tree.item(s[0])['values'][3] if self.tree.selection() else self.dst_tree.item(s[0])['values'][3]; self.apply_filter()
    def reset_filters(self): self.trace_node_ip = None; self.search_var.set(""); self.apply_filter()
